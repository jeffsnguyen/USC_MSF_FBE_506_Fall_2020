...
```{r}
# Graph $Y = .5^{T}
curve(.5^x, from = -8, to = 6, ylab = "Y", xlab = "T", main = "Y = .5^T")
```
...
```{r}
# Graph $Y = 2^{T}
curve(2^x, from = -5, to = 6, ylab = "Y", xlab = "T", main = "Y = 2^T")
```
...
```{r}
# Graphing monthly S&P 500 returns
chartSeries(returns_monthly_gspc, name = "Monthly Returns to S&P 500", type = "candlesticks", theme = chartTheme("white"))
```
...
```{r}
# Graphing monthly AAPL returns
chartSeries(returns_monthly_aapl, name = "Monthly Returns AAPL", type = "candlesticks", theme = chartTheme("white"))
```
...
```{r}
# Initialize xts objects contain close price for S&P 500 and AAPL and merge
gspc_xts <- as.xts(monthly_gspc[,"GSPC.Close"])
aapl_xts <- as.xts(monthly_aapl[,"AAPL.Close"])
price_compare <- merge.xts(gspc_xts, aapl_xts)

# Graph monthly AAPL and monthly S&P500 on one coordinate system
# Plot S&P 500
plot(as.zoo(price_compare[, "GSPC.Close"]), screens = 1, main = "S&P 500 and APPL closing price overlay", 
...
plot(as.zoo(price_compare[, "AAPL.Close"]), screens = 1, xaxt = "n", yaxt = "n", xlab = "", ylab = "", col = "Blue")

# Add right-handed axis to display AAPL price
axis(4)

# Add legend
legend("topleft", c("S&P 500", "AAPL"), lty = 1:3, cex = 0.75, fill = c("red", "blue"))
```
...
returns_gspc_xts <- as.xts(returns_monthly_gspc[,"GSPC.Close"])
returns_aapl_xts <- as.xts(returns_monthly_aapl[,"AAPL.Close"])
...
plot(as.zoo(returns_compare[, "GSPC.Close"]), screens = 1, main = "S&P 500 and APPL closing price overlay", 
...
plot(as.zoo(returns_compare[, "AAPL.Close"]), screens = 1, xaxt = "n", yaxt = "n",
     xlab = "", ylab = "", col = "Blue")
...
# Add legend
legend("bottom", c("S&P 500 Returns", "AAPL Returns"), lty = 1:3, cex = 0.75, fill = c("red", "blue"))
```
